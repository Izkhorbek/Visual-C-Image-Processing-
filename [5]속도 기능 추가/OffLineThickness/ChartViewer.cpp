...
void CChartViewer::updateViewPort(bool needUpdateChart,	bool needUpdateImageMap)
{
	...
	GetParent()->SendMessage(WM_COMMAND, MAKEWPARAM(GetDlgCtrlID(), CVN_ViewPortChanged2),
		(LPARAM)m_hWnd);
	
	GetParent()->SendMessage(WM_COMMAND, MAKEWPARAM(GetDlgCtrlID(), CVN_ViewPortChanged3),
		(LPARAM)m_hWnd);
...
void CChartViewer::Initialize()
{
	CRect rt;

	GetWindowRect(&rt);
	ScreenToClient(&rt);
	m_btnRadio1.Create(_T("라디오 버튼1"), WS_CHILD | WS_VISIBLE | WS_GROUP | BS_AUTORADIOBUTTON,
		CRect(5,105,15,115), this, 104);
	m_btnRadio1.ShowWindow(SW_SHOW);
